...
2. 地図上の範囲で百名店を絞り込める
3. 抽出した店舗を予約状態ごとに区分けできる
4. 指定日、人数、時間帯で予約可能店舗だけを抽出できる
5. 予約対象外店舗と予約終了店舗を区別して扱える
...
2. 地図範囲で静的検索できる
3. `reservation_status` なし検索では外部照会なしで結果を返せる
4. `reservation_status = bookable` 検索で予約可能店舗を返せる
5. `not_supported` と `sold_out` を別区分で返せる
6. 手動確認対象を CSV で出力できる
