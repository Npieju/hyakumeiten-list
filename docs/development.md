...
このドキュメントは、このリポジトリを更新するときの開発者向けメモです。公開向けの概要は [README.md](/home/yt/Projects/hyakummeiten-list/README.md) を参照してください。
...
環境によっては `python3 -m venv` や `ensurepip` が使えない場合があります。その場合は、利用可能な Python 環境で必要パッケージを導入してください。
...
この集計では、同じ `Website` の店舗は 1 行にまとめます。`Year`、`Genre`、`Genre Slug`、`Release Date` は ` | ` 区切りの複数値になります。
